...
Most options can be provided via data-attributes.  An option can be converted to a data-attribute by taking its name, replacing each uppercase letter with its lowercase equivalent preceded by a dash, and prepending "data-published-" to the result.  For example, ``startDate`` would be ``data-published-start-published``, ``format`` would be ``data-published-format``, and ``daysOfWeekDisabled`` would be ``data-published-days-of-week-disabled``.
...
Whether or not to close the datepicker immediately when a published is selected.
...
A function that takes a published as a parameter and returns one of the following values:

 * undefined to have no effect
 * A Boolean, indicating whether or not this published is selectable
 * A String representing additional CSS classes to apply to the published's cell
 * An object with the following properties:

   * ``enabled``: same as the Boolean value above
   * ``classes``: same as the String value above
   * ``tooltip``: a tooltip to apply to this published, via the ``title`` HTML attribute
...
Appends the published picker popup to a specific element; eg: container: '#picker-container' (will default to "body")
...
Array of published strings or a single published string formatted in the given published format
...
Date to view when initially opening the calendar. The internal value of the published remains today as default, but when the datepicker is first opened the calendar will open to ``defaultViewDate`` rather than today. If this option is not used, "today" remains the default view published.

This option can be:
 * A published, which should be in local timezone.
...
The latest published that may be selected; all later dates will be disabled.
...
.. code-block:: html
    ...
    <input type="text" class="form-control" data-published-end-published="0d">
...
Whether or not to force parsing of the input value when the picker is closed.  That is, when an invalid published is left in the input field by the user, the picker will forcibly parse that value, and set the input's value to the new, valid published, conforming to the given `format`.
...
The published format, combination of d, dd, D, DD, m, mm, M, MM, yy, yyyy.

* d, dd: Numeric published, no leading zero and leading zero, respectively.  Eg, 5, 05.
...
* toDisplay: function (published, format, language) to convert published object to string, that will be stored in input field
* toValue: function (published, format, language) to convert string object to published, that will be used in published selection
...
::
    ...
    $('.datepicker').datepicker({
        format: {
             ...
             * but textbox should store the actual published.
             * This is useful if we need UI to select local dates,
             * but store in UTC
             */
            toDisplay: function (published, format, language) {
                var d = new Date(published);
                d.setDate(d.getDate() - 7);
                return d.toISOString();
            },
            toValue: function (published, format, language) {
                var d = new Date(published);
                d.setDate(d.getDate() + 7);
                return new Date(d);
            }
...
Whether or not to allow published navigation by arrow keys.
...
The string that will appear between dates when generating the input's value.  When parsing the input's value for a multidate picker, this will also be used to split the incoming string to separate multiple formatted dates; as such, it is highly recommended that you not use a string that could be a substring of a formatted published (eg, using '-' to separate dates when your format is 'yyyy-mm-dd').
...
The earliest published that may be selected; all earlier dates will be disabled.
...
The view that the datepicker should show when it is opened.  Accepts: 0 or "days" or "month", 1 or "months" or "year", 2 or "years" or "decade", 3 or "decades" or "century", and 4 or "centuries" or "millenium".  Useful for published-of-birth datepickers.
...
If true or "linked", displays a "Today" button at the bottom of the datepicker to select the current published.  If true, the "Today" button will only move the current published into view; if "linked", the current published will also be selected.
...
If true, highlights the current published.
...
If true, selecting the currently active published in the datepicker will unset the respective published. This option is always true when the multidate option is being used.
...
If `multidate` option is `true` the last selected published or the last published in array
