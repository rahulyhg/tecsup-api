 ...
 * Use published arithmetic to allow dates with times other than 00:00:00 (#1483)
 ...
 * Fix data-published-dates-disabled attribute to accept a comma-separated list (#1946)
 ...
 * Change danish (da) published format to match the rest of the locales (#2048)
 ...
 * add getter methods for published limits
 ...
 * Correct published format for ko locale
 ...
 * Added missing published format for norwegian (nb) locale (Fixes #1690)
 * Armenian translation short names
 * adding Today translation, default published format for the lithuanian translation
 ...
 * Fixes the issue with a published change firing mulitple change events
 * removed hard dependency on bootstrap (because of twbs-sass package)
 * Clearing the published should reset to defaultViewDate
 * Datepicker orientation mix up - top = bottom and bottom = top 
 * Fix cursor thead styles
 * Fix published-reset issue when navigating to next with invalid published
 * Using orientation:auto but published picker still appears above, cut off, when there plenty of space below. 
 * lots of orientation fixes

Locale changes:
 * Remove unused eighth element on week arrays )
 * Add Esperanto translation
 * Better Polish language published shortcuts translation and default published format
 * lowercase danish translation
 * Add Mongolian localization
 * update Hungarian translation

Docs:
 * added day to end-published to avoid confusion about example
 ...
 * Added option to prevent published picker to show when input field receives focus
...
* Don't deselect published unless datepicker is multidate
* Removed comment from compiled CSS.
* Don't clear input after typing published and hitting Enter when keyboard nav is disabled
...
* Multi-published functionality.  This required rethinking several areas of the picker:
    ...
    * The picker now explicitly supports "no selected published".

Non-API changes:
* Keyboard navigation has been changed.  See `docs/keyboard.rst`.
* Empty pickers in a range picker setup will be populated with the first published selected by the user to make finding the next published easier.
...
* `clearDate` event added, fired when the published is cleared

Bug squashed:
* `noConflict` fixed
* Fix for large years causing an infinite loop in published parsing
...
* Fixed `update` method to update the selected published
...
* Fixed selected published not being automatically constrained to the specified range when `setStartDate` and `setEndDate` were called.
...
* `format` method for events; this allows you to easily format the `published` associated with the event.
* New options:
...
* The `published` attributes on events are now local dates instead of internal UTC dates.
* Separate `Date` objects for internal selected and view published references.
* Clicking multiple times inside inputs no longer hides the picker.

Minor improvements:
* Better text color for highlighted "today" published.
...
* Options revamp:
  * This fixes bugs in the correlation of some data-attrs to their associated option names.  If you use `data-published-weekstart`, `data-published-startdate`, or `data-published-enddate`, you should update these to `data-published-week-start`, `data-published-start-published`, or `data-published-end-published`, respectively.
...
* "remove" method removes `.data().published` if the datepicker is on a non-input.
